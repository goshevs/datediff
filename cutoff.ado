...
program define dateForward, sclass
	...
	*** Move forward days
	if ("`type'" == "age") {
		...
		gen `day_excess' = cond(day(`bdate') == `daysInMonth', 1, abs(day(`bdate') - `daysInMonth'))

		*** February and first of month adjustment
		gen `gen' = cond(month(`ym') == 2 & day(`bdate') ~= 1, ///
						`ym' + day(`bdate') + `days' - `day_excess' - 1, ///
						`ym' + day(`bdate') + `days' - 2)
	}
	else {
		*** Case in which type is not age -- TEST THIS!
		gen `day_excess' = cond(day(`bdate') == `daysInMonth', 0, abs(day(`bdate') - `daysInMonth'))

		*** February and first of month adjustment
		gen `gen' = cond(month(`ym') == 2 & day(`bdate') ~= 1, ///
						`ym' + day(`bdate') + `days' - `day_excess', ///
						`ym' + day(`bdate') + `days' - 1)
	}
